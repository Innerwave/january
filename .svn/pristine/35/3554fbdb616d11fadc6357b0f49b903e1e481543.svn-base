/*
 * Innerwave's jQuery Spreadsheet plugin v0.2.1226-SNAPSHOT
...
( function ( factory ) {
  ...
  function offsetRight( element ) {
    return Math.round( element.offsetLeft + element.offsetWidth ) - parseInt( $( element ).css( "border-right-width" ).replace( "px", "" ) || 1 );
  }

  function offsetBottom( element ) {
    return Math.round( element.offsetTop + element.offsetHeight ) - parseInt( $( element ).css( "border-bottom-width" ).replace( "px", "" ) || 1 );
  }

  /**
   * 엑셀 스타일의 컬럼 이름(타이틀)을 생성
  ...
  function Sheet() {
    ...
    this.addColumns = function ( columnInfos ) {
      ...
      for ( var i = 0, l = columnInfos.length; i < l; i++ ) {
        columnInfos[ i ].visualId = getColumnHeaderText( i );
  ...
  function ColumnGroup( info ) {
    ...
    this.columns = new ColumnCollection();
    this._ui = info.ui || $( "<li class='spreadsheet-column spreadsheet-column-group ui-state-default'><span>" + this.label + "<span></li>" )
      .button()
      ...
      .resizable( {
        ...
        minWidth: 2,
        stop: function ( e, ui ) {
          var cg = $( this ).data( "group" );
          var dw = Math.floor( ( ui.size.width - ui.originalSize.width ) / cg.columns.length );
          var width = 0;
          cg.columns.each( function ( i ) {
            var w = cg.columns.length === i + 1 ? ui.size.width - ui.originalSize.width - width : dw;
            this.width = this.ui.width() + w;
            this.ui.width( this.width );
            width += w;
          } );
          cg.parent.parent._renderColumns( cg.parent.parent.boundary.firstColumn );
        }
      } );
  ...
  function Column( columnInfo ) {
    ...
    this.uuid = columnInfo.id || uniqueId( "column" );
    this.id = columnInfo.id || this.uuid;
    this.visualId = columnInfo.visualId;
  ...
  Column.prototype.offset = function () {
    if ( this.ui ) {
      this._offset = {
  ...
  function Row( rowInfo ) {
    rowInfo = rowInfo || {};
    this.uuid = rowInfo.id || uniqueId( "row" );
  ...
  function Cell( cellInfo ) {
    cellInfo = cellInfo || {};
    this.uuid = cellInfo.id || uniqueId( "cell" );
  ...
  */
  ...
  return $.widget( "iui.spreadsheet", {
    version: "0.2.1226-SNAPSHOT",
    options: {
      // buttons: [],
      // spreadsheetClass: "",
      height: "auto",
    ...
    _parseEachData: function ( data, row ) {
      ...
      $.each( data, function ( key ) {
        if ( key === "id" || key === "uid" || key === "uuid" || key === "height" || key === "width" ) {
          row.key = this;
          return;
        }
        ...
        if ( cellInfo.rows ) {
          for ( i = 0, l = cellInfo.rows.length; i < l; i++ ) {
            item = that.sheet.rows.getById( cellInfo.rows[ i ] );
        ...
        if ( cellInfo.columns ) {
          for ( i = 0, l = cellInfo.columns.length; i < l; i++ ) {
            item = that.sheet.columns.getById( cellInfo.columns[ i ] );
            if ( item && $.inArray( cell.columns, item ) < 0 ) {
              cell.columns.push( item );
            }
          }
        }
    ...
    _render: function () {
      ...
      this.$uiHeader = $( "<div class='spreadsheet-header ui-state-default'>" )
        .appendTo( this.$uiContainer );
      this.$uiViewport = $( "<div class='spreadsheet-viewport ui-widget-content'>" )
        .appendTo( this.$uiContainer );
      this.$defaultGrid = $( "<canvas class='spreadsheet-gridlines'>" )
        .appendTo( this.$uiViewport );
      this.$uiColumnViewport = $( "<div class='spreadsheet-viewport-column ui-state-default'>" )
        .appendTo( this.$uiContainer );
    ...
    _createUiColumn: function ( i, column ) {
      ...
      return column.ui = $( '<li>' )
        ...
        .mousedown( function ( event ) {
          var uis = [];
          var column = that.sheet.columns.getById( this.id );
          if ( column.group ) {
      ...
      function render() {
        ...
        for ( ; i < l && columnsWidth < this.$uiViewport.width(); i++ ) {
          ...
          if ( this.options.showColumnGroups ) {
            ...
            group = this.sheet.columnGroups.getById( groupid );
            if ( !group ) {
              group = new ColumnGroup( {
                id: groupid,
                label: column.group
              } );
              group.parent = this.sheet;
              group.columns.add( column );
              this.sheet.columnGroups.add( group );
            }
            groups.push( group );
          }
        }
