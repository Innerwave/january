(function ($) {
  ...
  QUnit.test('생성', function (assert) {
    ...
    assert.equal($uiSheet.spreadsheet('rows').length, 0, '기본 생성한 Row의 수는 0이다');
    assert.equal($uiSheet.spreadsheet('columns').length, 0, '기본 생성된 Column의 수는 0이다');
    assert.equal($uiSheet.spreadsheet('data').length, 0, '기본 생성한 Data의 수는 0이다');

    var columnSize = 50;
    columns = createColumns(columnSize);

    $uiSheet.spreadsheet('columns', columns);
    assert.equal($uiSheet.spreadsheet('columns').length, columnSize, '주입한 Column의 수는 50이다');
    ...
    assert.equal($uiSheet.spreadsheet('data').length, dataParseUnit, '주입한 Data의 수는 ' + dataParseUnit + '이다');
    // 100 개만 반환된다. setTimeout 사용에 따른 문제인것 같다.
    assert.equal($uiSheet.spreadsheet('rows').length, dataParseUnit, 'Data 주입으로 인해 생성한 Row의 수는 ' + dataParseUnit + '이다');
